...
4. [Арифметика](#арифметика)
5. [Переменные](#переменные)
6. [Массивы](#массивы)
7. [Управляющие конструкции](#управляющие-конструкции)
    - [Условия](#условия)
    - [Циклы](#циклы)
8. [Процедуры](#процедуры)
9. [Пользовательские типы](#пользовательские-типы)
10. [Дженерикики](#дженерик-типы)
...
Нельзя создать непустой тип массива для таких типов как any или array.
Для обохода этого ограничения существует функция makeArray.
...
В языке поддерживаются дженерик типы, которые позволяют создавать типы с параметризованными полями.  
Объявит дженерик тип можно следующим образом:
```plamp
type Container[T] {
    field: T;
}
...
T будет дженерик параметром типа.

Далее такой тип может быть использован в коде. Вместо параметра следует подставить конкретный тип.
```
...
Можно создавать экземпляры подобного типа
...
В таком случае значение параметризованных полей будет иметь значение по умолчанию для типа аргумента.

Тип может иметь несколько дженерик параметров.
...
Не только типы могут быть параметризованы, но и функции. Для параметризации функции следует указать имя дженерик параметра следом за именем функции в квадратных скобках. 
Если требуется указать несколько параметров, то они перечисляются через запятую.

```
//Эта функция получит и вернёт то же значение
fn generic_function[T](a: T) T {
    return a;
}
```

Дженерик функции и дженерик типы можно комбинировать друг с другом.

```
//Эта функция получит контейнер и вернёт значение лежащее в нём.
fn unwrap[T](container: Container[T]) T {
   return container.field;
}
```

Также внутри дженерик функций можно создавать дженерик массивы, но следует учесть, что они могут быть только пустыми.
...
//Вернуть дженерик массив ненулевой длины нельзя
fn mk_array[T]() []T {
   return [10]T;
}
```

Чтобы решить такую проблему есть функция, которая способна создать дженерик массив требуемой длины.

Вызвать дженерик функцию можно двумя способами. Передавая дженерик аргументы явно или неявно.
```
fn generic[T](arg: T) {}

fn main(){
   generic[int](1); //Явная передача
   generic(1); //Неявная передача
}
```
Явный вызов функции удобен, но в некоторых случаях функцию вызвать таким образом нельзя.

```
fn generic[T]() []T { return []T{}; }

fn main(){
   a := generic(); //Будет получена ошибка
   b := generic[int](); //Корректный вызов функции
}
```

Дженерик внутри функции нельзя создать
```
fn make_type[T]() T { return T{};} // Не скомпилируется
```
...
* `arrLen(array) int` - возвращает длину массива
* `strLen(string) int` - возвращает длину строки
* `read() char` - читает один символ
* `readln() string` - читает одну строку до переноса
* `strConcat(string, string) string` - соединяет строки воедино
* `arrConcat([]string) string` - соединяет массив строк воедино
* `makeArray[T](T, int), []T` - создаёт массив указанной длины повторяя значение первого параметра.
